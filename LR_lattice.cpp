...
//	V_av = new double[size*size];
	...
	if (ProcRank != ProcNum - 1)
	{
		...
		for (int i=0; i < N-2; i++)
		{
			read(file_id,&(V[N+i*N+1]),(N-2)*sizeof(double));
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].m),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].h),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].j),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].d),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].f),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].X),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
		...
		{
			for (int j=1;j<N-1;j++)
				read(file_id,&(Cell[i*N+j].Cai),sizeof(double));
			_lseek(file_id, (Size-N+2)*sizeof(double), SEEK_CUR);
		}
	}
	close(file_id);
