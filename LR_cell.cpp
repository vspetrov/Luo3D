...
double VFunction(int &ii, int &jj, int &kk) //find V for the Cell numbered CN_c, CN_r
{
    ...
    s_tension = Cell[(ii+kk*N)*N+jj].s_tension;
    tension = Cell[(ii+kk*N)*N+jj].tension;
    ...
    I_SAC =  G_s_mech * sigmoid(tension)*tension*(U - E_mech_threshold);

    return -(i_Na+i_si+i_K+i_Kl+i_Kp+i_b+I_ext[(ii+kk*N)*N+jj] + I_SAC);
}
