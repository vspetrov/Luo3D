...
int main(int argc, char *argv[])
{
	...
	t1 = MPI_Wtime();
	//создание коммуникаторов для вычислительной и записывающей в файл групп процессов
	...
	else
	{
		...
		for (int i=0;i<DrawNum;i++)
		{
			...
			save(V_save,(N-2)*(N-2)*(ProcNum-1),fd);//ProcNum-1 чтобы массив несчетного процесса(Rank=ProcNum-1) не сохранять
			}
